...
router.post('/:follower', async (req, res, next) => {
    ...
    if (ID != -1) {
        ...
        let myCount = await db.execute3(follow, ID, from_idx);

        console.log(myCount);

        if(myCount[0].following_count != 0){
            res.status(405).send({
                message: "Already Following"
            });
        }else{
            if(count[0].following_count === 0){
                let data = await db.execute2(QUERY, [ID, from_idx, follower, 0]);
                ...
                let data = await db.execute2(QUERY, [ID, from_idx, follower, 1]);
